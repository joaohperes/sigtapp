      ...
      <main className="mx-auto max-w-7xl px-6 py-6">
          ...
          <div className="flex gap-6">
            ...
            {gruposPresentes.length > 1 && (
              <aside className="hidden lg:block w-56 shrink-0">
                <div className="sticky top-[60px] overflow-hidden rounded-xl bg-card border border-border shadow-sm">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
                <p className="text-sm text-muted-foreground">
                  ...
                  {hasMore && <span className="text-muted-foreground/60"> · mostrando {visibleCount}</span>}
                </p>
              ...
              {showFilters && (
                <div className="mb-4 rounded-xl border border-border bg-card p-5 shadow-sm space-y-5">
                  ...
                  <label className="flex cursor-pointer items-center gap-2.5">
                    ...
                    />
                    <span className="text-sm text-foreground">Só procedimentos com descrição</span>
                  </label>
              ...
              {!loading && searched && results.length > 0 && sortedResults.length === 0 && (
                <div className="py-12 text-center">
                  <p className="text-sm font-medium text-foreground">Nenhum resultado com esses filtros</p>
                  <button onClick={resetFilters} className="mt-2 text-xs text-muted-foreground hover:text-foreground transition">
                    Limpar filtros
                  </button>
                </div>
              )}
        ...
        {!loading && searched && results.length === 0 && !error && searchMode !== 'cid' && (
          <div className="py-20 text-center">
            ...
            <p className="mt-1 text-xs text-muted-foreground">Tente outros termos ou verifique o código</p>
          </div>
        )}
            ...
            {favoritos.length > 0 && (
              <div className="mb-8">
                <div className="mb-3 flex items-center justify-between">
                  ...
                  <Link to="/favoritos" className="text-xs text-muted-foreground hover:text-foreground transition">
            ...
            {selectedGroup && (
              <div className={cn(
                ...
                <nav className="flex items-center gap-1.5 text-sm">
                  ...
                  {selectedSubgroup && (
                    <>
                      <svg className={cn("h-3.5 w-3.5 opacity-50", selectedEstilo?.text ?? "text-muted-foreground")} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                      </svg>
                      <span className={cn("font-semibold", selectedEstilo?.text ?? "text-foreground")}>
                ...
                >
                  <div className="overflow-hidden rounded-xl bg-card border border-border shadow-sm">
                    <div className="flex items-center gap-2 border-b border-border px-4 py-3">
                      ...
                      <span className="text-xs font-semibold text-foreground">{selectedGroup.no_grupo}</span>
                    </div>
              ...
              {selectedSubgroup && (
                ...
                >
                  <div className="overflow-hidden rounded-xl bg-card border border-border shadow-sm">
                    <div className="flex items-center gap-2 border-b border-border px-4 py-3">
                      <div className={cn("h-2.5 w-2.5 shrink-0 rounded-sm", selectedEstilo?.dot)} />
                      <p className="text-xs font-semibold leading-snug text-foreground">{selectedSubgroup.no_subgrupo}</p>
                    </div>
